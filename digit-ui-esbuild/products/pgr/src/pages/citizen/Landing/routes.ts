...
// Type-only elsewhere in the deck, but a real value import here: the hotline is
// county data, so it is owned by content.ts. content.ts imports this module with
// `import type`, which TypeScript erases — so there is no runtime import cycle.
...
export const DEFAULT_LANDING_ROUTES: LandingRoutes = {
  ...
  // External / telephony channels. WhatsApp is not a pilot channel for Bomet,
  // which also removes the floating WhatsApp action — WhatsAppFab renders
  // nothing for "#".
  WHATSAPP: "#",
  // County hotline, supplied 2026-08-15. Derived from CONTACT.hotline so the
  // number is edited in exactly one place (content.ts) — see the note there.
  GREEN_LINE: `tel:${CONTACT.hotline}`,
  PHONE: `tel:${CONTACT.hotline}`,
};
